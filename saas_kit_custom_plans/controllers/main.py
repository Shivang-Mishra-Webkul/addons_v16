...
class PlanPage(http.Controller):
    ...
    def _create_custom_plan_data(self, *ver):
        ...
        data['odoo_version'] = version_code if version_code else request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm')]) if data['is_odoo_version'] else request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm'), ('code', '=', '16.0')], limit=1)
        ...
        data['odoo_version'] = request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm')]) if data['is_odoo_version'] else request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm'), ('code', '=', '16.0')], limit=1)
...
class Website(Website):
    ...
    # @http.route(['/saas/version/data'], type='json', auth="public", website=True)
    @http.route(['/saas/version/data'], type='http', auth="public", website=True)
    def get_plans_data(self, **post):
    ...
    def get_community_plans_data(self, version_id=None, **kw):
        ...
        if version:
            ...
            # if version.is_enterprise:
            #     v = str(version.code).split('.')
            #     version_code = f"{v[0]}e.{v[1]}" if v else "16.0"
            ...
            values['plans'] = plans
        if not plans:
            values['message'] = f"No {'community' if not 'e' in version.code else 'enterprise'} plans found for this version."

...
class odoo_container:
    def  __init__(self,db="dummy",odoo_image="odoo:12.5",odoo_config = None,host_server = None, db_server = None, version = "16.0", is_enterprise = False):
#        self.odoo_image = odoo_image
        ...
        self.response = {}
        self.read_variables(self.location+"/models/lib/saas.conf",is_enterprise)

    def read_variables(self,path,is_enterprise):
        ...
        if is_enterprise:
            self.enterprise_addons_path = parser.get("options","enterprise_addons_v"+version)
...
#            os.mkdir(path)
    ...
    def run_odoo(self,name, db, is_enterprise = False):
    ...
    def update_config_paramenter(self,file_path=None, parameter=None, value=None):
        ...
        try:
            ...
            # Get Parameter
            param_val = config.get('options', parameter)
            _logger.info(f"-----------param_val------{param_val}-----------")
            
            # Update the parameter
            new_value = param_val+','+value
...
def main(context=None):
    ...
    is_enterprise = True if odoo_version in ["15e.0","16e.0","17e.0"] else False
    ...
    port = OdooObject.run_odoo(host_domain, db, is_enterprise = is_enterprise)
    if not port:
...
def create_db_template(db_template=None,modules=None, config_path=None,host_server = None, db_server = None, version = "16.0", is_enterprise = False):
    _logger.info(locals())
    ...
    OdooObject = odoo_container(db=db_template,odoo_config = config_path, db_server = db_server, host_server = host_server, version = version, is_enterprise = is_enterprise)
    OdooObject.get_client()

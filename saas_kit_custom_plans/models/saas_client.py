            ...
            try:
                ...
                if cred_response.get('status'):
                    ...
                    response = saas_install.main(dict(
                        ...
                        login=login,
                        password=password,
                        version=obj.saas_contract_id.odoo_version_id and obj.saas_contract_id.odoo_version_id.code or '16.0')
                    )
                    if response:
            ...
            if obj.saas_contract_id.is_custom_plan:
                try:
                    if obj.state == "inactive":
                        ...
                        response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('odoo_saas_kit'), from_drop_db=True, version=self.saas_contract_id.odoo_version_id.code or '16.0')
        ...
        for obj in self:
            if obj.saas_contract_id.is_custom_plan:
                if obj.state == "inactive":
                    host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
                    _logger.info("HOST SERER %r   DB SERVER  %r"%(host_server,db_server))
                    response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('odoo_saas_kit'), container_id=obj.container_id, db_server=db_server, from_drop_container=True, version=self.saas_contract_id.odoo_version_id.code or '16.0')

...
class ContractCustomPlan(models.Model):
    ...
    def get_code(self):
        for rec in self:
            ...
            # if rec.odoo_version_id.code and 'e' in rec.odoo_version_id.code:
    ...
    def set_user_data(self, token=False):
        for obj in self:
            ...
            """
            extra_modules = self.env['saas.module'].sudo().search([('is_published', '=', True), ('id', 'not in', obj.saas_module_ids.ids), ('odoo_version_id.code', '=', obj.odoo_version_id.code or '16.0')])
    ...
    def add_apps(self, apps, contract_id):
        ...
        modules = self.env['saas.module'].sudo().search([('technical_name' , 'in', apps), ('odoo_version_id.code', '=', contract_id.odoo_version_id.code or '16.0')])
        ...
        extra_modules = self.env['saas.module'].sudo().search([('is_published', '=', True), ('id', 'not in', modules_list), ('odoo_version_id.code', '=', contract_id.odoo_version_id.code or '16.0')])
        extra_modules = [x.id for x in extra_modules]
    ...
    def get_module(self, contract_id):
        ...
        """
        contract_id = self.env['saas.contract'].sudo().browse([contract_id])
        modules = self.env['saas.module'].sudo().search([('is_published', '=', True), ('odoo_version_id.code', '=', contract_id.odoo_version_id.code or '16.0')])        
        extra_modules = []
        for module in modules:

...
class SaasPlanCustomPlan(models.Model):
    ...
    def _compute_odoo_version_code(self):
        for version in self:
            version.odoo_version_code = version.plan_odoo_version.code if (version.provide_odoo_version and version.plan_odoo_version) else '16.0'
            #New flow for enterprise edition
            version.is_enterprise = version.plan_odoo_version.is_enterprise if (version.provide_odoo_version and version.plan_odoo_version) else False

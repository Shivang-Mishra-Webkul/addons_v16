...
class SaasOdooVersion(models.Model):
    ...
    name = fields.Char(string="Title", help="""Display Title/Name for the Odoo Version""")
    version_code = fields.Many2one(comodel_name="saas.version.code", string="Version Code", help="""Enter the version code for this Odoo Version. For eg. - 15.0/16.0""", domain="[('is_enterprise','=', is_enterprise)]", required=True)
    code = fields.Char(string="Version Code", related="version_code.code", help="""Enter the version code for this Odoo Version. For eg. - 15.0/16.0""")
    state = fields.Selection(selection=STATE, string="State", default="open", help="""Choose the state of the Odoo Version""")
    is_template_created = fields.Boolean(string="Template Created", default=False, help="""Checked if the DB Template of this Odoo Version is created else Unchecked""")
    db_template = fields.Char(string="DB Template", compute="_compute_db_template_name", store=True, help="""Name of the DB template of the Odoo Version is computed automatically as per the odoo version title.""")
    product_id = fields.Many2one(comodel_name="product.product", string="Template Product", domain="[('is_saas_module', '=', True)]", help="""Select the product for this Odoo Version which will be used in purchasing the modules from the website.""")
    ...
    is_multi_server = fields.Boolean(string="Deploy Client's on Remote Server", default=False, help="""Select if you want to deploy the saas clients of this Odoo Version on the Remote Server.""")
    default_saas_servers_ids = fields.One2many(comodel_name="server.priority", inverse_name="saas_version_id", help="""Add the remote server for this Odoo Version along with their priority. The saas clients are created on the remote server based on the priority.""")
    is_drop_db = fields.Boolean(string="Is DB Dropped", default=False, help="""Checked if the database of this Odoo Version is dropped.""")
    is_enterprise = fields.Boolean(string="Enterprise Edition", default=False, help="""Select if you want to create enterprise edition saas clients for this Odoo Version.""")
    enterprise_addons_path = fields.Char(string="Enterprise Addons Path")
    path_verified = fields.Boolean(string="Path verified")

    
    @api.onchange('is_enterprise')
    def update_version_code(self):
        for rec in self:
            # if rec.is_enterprise:
            #     rec.version_code = None
            # if rec.is_enterprise == False:
            #     rec.version_code = None
            rec.version_code = None
    ...
    def get_default_saas_values(self):
        ...
        data['odoo_version'] = self.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm')]) if data['is_odoo_version'] else self.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm'), ('code', '=', '16.0')], limit=1)
        ...
        data['discount_percent'] = float(IrDefault.get('res.config.settings', 'discount_percent') or 0.00)
    ...
    def write(self, vals):
        _logger.info(vals)
        if vals.get('enterprise_addons_path'):
    ...
    def check_enterprise_addons_path(self):
        try:
            exists = False
            if os.path.isdir(self.enterprise_addons_path):
                module_path = os.path.join(self.enterprise_addons_path, "web_enterprise")

...
odoo.define('saas_kit_custom_plan.product_page', function (require) {
    ...
    publicWidget.registry.CustomPlan = publicWidget.Widget.extend({
        ...
        _check_discount : function(){
            ///////////////////Annual Discount////////////////////
            if (annual_discount){
                // alert("in annual discount");

                if (number == 12){
                    // alert("in yearly annual discount");
        ...
        _onSelectVersion : function(event){
            ...
            // console.log($(event.target).attr('version_code'));
            
            if ($('#toggle_input').is(':checked')){
                console.log("IN redirect cat ver");
                ajax.jsonRpc("/custom/version/categ", 'call', {
                    'version_code': version_code,
                }).then(function(data){
                    $('#category_view_main_div').replaceWith(data);
                    // console.log("redirect cat ver");
                });
            }

            else{
                console.log("IN redirect normal ver");
                ajax.jsonRpc("/custom/version/normal", 'call', {
                    'version_code': version_code,
                }).then(function(data){
                    $('#normal_view_main_div').replaceWith(data);
                    // console.log("redirect normal ver");
                });
            }
            $.get("/show/selected/apps/view", {
            }).then(function (data) {

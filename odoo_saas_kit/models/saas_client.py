...
class SaasClient(models.Model):
    ...
    container_space_usage = fields.Char(string="Container Space Usage", default="To be fetched!!")
    ...
    #fetch_container_usage
    def fetch_container_usage(self):
        for obj in self:
            host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
            try:
                dock_sz_resp = client.fetch_container_resource(host_server, config_path = get_module_resource('odoo_saas_kit'), container_name = obj.container_id, action = "memory")
                filestore_size = dock_sz_resp.get("FilestoreUsage")
                container_size = dock_sz_resp.get("ContainerUsage")
                _logger.info(f"======fetch_container_usage==0==={dock_sz_resp}=={filestore_size}=={container_size}===")

                db_sz_resp = query.check_database_size(obj.database_name, db_server)
                _logger.info(f"======fetch_container_usage==1==={db_sz_resp}=======")

                db_size = int(((db_sz_resp.get("result"))[0][0]).split(" ")[0])
                _logger.info(f"======fetch_container_usage================")

                obj.container_space_usage = str(filestore_size + container_size + db_size) + " MB   (Filestore Usage: " + str(filestore_size) +" MB,  Container Usage: " + str(container_size) + " MB,  Database Usage: " + str(db_size) + " MB)"
            except Exception as e:
                raise UserError(f"{e}")

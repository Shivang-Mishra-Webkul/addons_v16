...
#        return used_ports
    ...
    #         _logger.info(f"----------remote-param_val------{param_val}-----------")
    ...
    def get_remote_conf_parameter(self, file_path, parameter):
        try:
            ssh_obj =  self.login_remote()
            # cmd = "echo \"%s\" >> %s"%(value,file_path)
            cmd = '''pathToConf="%s"; param="%s"; while IFS= read -r line; do key=${line%% *} if [ $key == $param ]; then value=${line#* }; echo "$value" | xargs | sed 's/\=//g'; fi; done < "$pathToConf"'''%(file_path, parameter)
            return self.execute_on_remote_shell(ssh_obj,cmd)
        except Exception as e:
            _logger.error("Error appneding to file %r",e)
...
def create_db_template(db_template=None,modules=None, config_path=None,host_server = None, db_server = None, version = "16.0", is_enterprise = False, enterprise_addons_path = None):
    ...
    # is_enterprise = True if version in ["15e.0","16e.0","17e.0"] else False

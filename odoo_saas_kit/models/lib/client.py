...
def execute_on_shell_with_response(cmd):
    try:
        _logger.info("Saas Module Going to execute command ----    %r", cmd)
        res = subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=True)
        _logger.info("Saas Module successfully the command ----")
...
def get_container_size(container_name, host_server="localhost"):
    _logger.info(f"=========get_container_size====={container_name}==========")
    cmd_fr_container_inspect = f"docker inspect -s {container_name}"
    inspect_result = execute_on_shell_with_response(cmd_fr_container_inspect)
    inspect_result_list = inspect_result.decode()
    result_dict = (json.loads(inspect_result_list[1:-2]))
    _logger.info(f"====result_dict======={result_dict}======================")
    occupied_sz = result_dict.get("SizeRw")
    occupied_sz_in_mb = int((occupied_sz)/1048576)
    _logger.info(f"====occupied_sz_in_mb======={occupied_sz_in_mb}======================")
    return occupied_sz_in_mb


def fetch_container_resource(host_server, config_path=None, container_name=None, action=None):
    if action == "memory":
        parser = ConfigParser()
        parser.read(config_path + "/models/lib/saas.conf")
        odoo_config = parser.get("options","odoo_saas_data")
        path = odoo_config+'/'+container_name
        cmd = f"du -sh {path}"
        data_sz = execute_on_shell_with_response(cmd)
        _logger.info(f"======data_sz=========={data_sz}=======")
        data_sz_int = int(data_sz.decode().strip().split('/')[0][:-2])
        _logger.info(f"======data_sz=========={data_sz_int}=")
        cont_sz = get_container_size(container_name, host_server)
        _logger.info(f"======cont_sz=========={cont_sz}=======")

        response = {'ContainerUsage':cont_sz, 'FilestoreUsage':data_sz_int}
